...
def canPut(gamemap , x, y,player):# 0 => おける 1=>おけない  おけるか置けないか値を返します。
    rangeX = len(gamemap)
...
def changeStones(gamemap,x,y,turn): #石を置いて、裏返すときの処理
...
def render(gamemap,turn):#描画
    border_color = ""
    if turn == 1:
        border_color = f"{Color.BG_RED}"
    else :
        border_color = f"{Color.BG_BLUE}"
    mx_y = len(gamemap[0])
    print("\n" * terminal_size.lines)
    red_cnt = 0
    blue_cnt = 0
    ...
    render_cache = ""
    render_cache = f"{border_color}{Color.BLACK}  "
    for i in range(len(game_map)):
         render_cache +=f"{i} ";
    print(render_cache+f"x {Color.BG_DEFAULT}")
    for i in range(len(game_map)):
        render_cache =f"{border_color}{Color.BLACK} {i}{Color.WHITE}"
        if mx_y > 10 and i < 10:
            render_cache +=" "
        for j in range(len(game_map[i])):
            if gamemap[i][j] ==CellType.Player1:
                ...
                if canPut(gamemap,i,j,turn) ==0 and assist == True:
                    render_cache += f"{Color.BG_WHITE}[]{Color.BG_DEFAULT}"
                else :
                    render_cache += f"{Color.BG_BLACK}[]{Color.BG_DEFAULT}"
        print(render_cache+f"{border_color}  {Color.BG_DEFAULT}")
    render_cache = f"{border_color} {Color.BLACK}y";
    for i in range(len(gamemap)+1):render_cache +="  "
    print(render_cache+Color.BG_DEFAULT+Color.RESET)
